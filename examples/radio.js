...
const config = {
  credential:{
    serviceId: 'simpleapp', key: '27f8ca5ee273153d9a9c5944c766fa097d2e16de4dda425a'
  },
  sdk:{
    logLevel: 'VERBOSE',
    audioType: 'music'
  ...
  view: {
    local: 'localAudio', remote: 'remoteAudio',
  },
  media:{
    video: false,
    audio: {deviceId:'default', echoCancellation: false, autoGainControl:false, noiseSuppression: false}
  }
}
const config2 = {
  credential:{
  ...
  view: {
    local: 'localAudio2', remote: 'remoteAudio2'
  },
  audio: {remote: 'remoteAudio2'},
  media:{
    video: false,
    audio: {deviceId:'default', echoCancellation: false, autoGainControl:true, noiseSuppression: true}
  }
}
...
remon.on('onSearch', (list) => {
  ...
  if ((mode === 'CAST' || mode === 'VIEW')&& (list.length ==2) && livenum == 1) {
    livenum = 2
